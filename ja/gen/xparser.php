...
$PSY->livebase = "https://daidalvi.github.io/ja/";
...
$PSY->dir->content = "/home/xekima/Dropbox/SITES/Daidalvi/moon/memo";
$PSY->dir->site = "..";
$PSY->dir->assets = "assets";
$PSY->dir->wiki = "wiki";

$PSY->root  = $PSY->base.$PSY->dir->site."/";
$PSY->dbdir = $PSY->dir->content."/";
...
function parseDb(){
    ...
    foreach ($rows as $row) {
        ...
        if (strpos($cats, "O") !== false) {
            $data->tags.=" ОС";
        }
        if (strpos($cats, "Z") !== false && strpos($data->tags, "задача") === false) {
            $data->tags.=" задача";
        }
        if (strpos($cats, "F") !== false) {
            $data->tags.=" feature";
        }
...
function parseWiki(){
    ...
    if(!file_exists($PSY->wikidir."index.html")) psyError($txt);
...
function getMandala(){
    ...
    $imgFilesLoc = "https://daidalvi.files.wordpress.com/";
            ...
            foreach ($figures as $figure) {
                if (strpos($figure->getAttribute('class'), "wp-block-gallery") !== false) {
                    ...
                    foreach($imgs as $img){
                        ...
                        $segs = explode('.', $orig);
                        $ext = end($segs);
                        $imgName = str_replace(array(":", ",", "/"), "_", str_replace($imgFilesLoc, "", $orig)); // $k."_".$j.".".$ext
                        $newOrig = $PSY->wikidir."mandala/orig/".$imgName;
                        $newSrc = $PSY->livesite.$PSY->dir->wiki."/mandala/orig/".$imgName;
                        $newMini = ($ext=="jpg")? $PSY->livesite.$PSY->dir->wiki."/mandala/mini/".$imgName : $newSrc;
                        $imgData = str_replace($orig, $newSrc,$imgData);
                        if($PSY->saveImages){
                            file_put_contents( $newOrig, file_get_contents($orig));
                        }
                        $html .= "<li><a data-fancybox=\"images\" href='".$newSrc."'><img src='".$newMini."' /></a></li>";
                        $j++;
                    }
                    $html .= "</ul>";
                    $k++;
